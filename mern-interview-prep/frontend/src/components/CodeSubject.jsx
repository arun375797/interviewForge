...
import { Check, ChevronRight, Circle, Code2, Filter, Search } from 'lucide-react';
...
export default function CodeSubject() {
  ...
  const refreshTopics = () =>
    api.getCodeTopics({ subject }).then((data) => setTopics(data)).catch(console.error);
  ...
  useEffect(() => {
    ...
    if (filterMode === 'pending') params.completed = 'false';
    if (filterMode === 'done') params.completed = 'true';
  ...
  const topicPct = activeTopic?.count
    ? Math.round(((activeTopic.completed || 0) / activeTopic.count) * 100)
    : 0;

  const onLanguageChange = (key) => {
    navigate(`/code/${key}`, { replace: true });
  };
  ...
  const openQuestion = (id) => {
    navigate(`/code/${subject}/${id}`);
  };

  const toggleCompleted = async (q, e) => {
    e?.stopPropagation?.();
    if (togglingId) return;
    setTogglingId(q._id);
    try {
      const updated = await api.toggleCodeCompleted(q._id);
      setQuestions((prev) =>
    ...
    <div className="animate-fade space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-muted">
            Practical questions only. Tick completed problems and resume the pending ones later.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,280px)_minmax(0,1fr)]">
          ...
          <div className="max-h-[min(40vh,300px)] overflow-y-auto p-2 lg:max-h-[52vh]">
            {loading
              ...
              : filteredTopics.map((t) => {
                  const pct = t.count ? Math.round(((t.completed || 0) / t.count) * 100) : 0;
                  return (
                    <button
                      key={t.name}
                      type="button"
                    ...
                    >
                      ...
                      <div className="mt-2 flex items-center justify-between gap-2">
                        <p className={`text-xs ${topic === t.name ? 'text-paper/70' : 'text-muted'}`}>
                          {t.completed || 0}/{t.count} done · {pct}%
                        </p>
                        <div
                        ...
                        >
                          <div
                            className={`h-full rounded-full ${
                              topic === t.name ? 'bg-paper' : 'bg-accent'
                            }`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
        ...
        <div className="min-w-0 space-y-4">
          {topic && activeTopic ? (
            <div className="glass-panel rounded-2xl p-4">
              ...
              <p className="mt-1 text-sm font-medium">
                {activeTopic.completed || 0} of {activeTopic.count} completed ({topicPct}%)
              </p>
          ...
          <div className="glass-panel flex flex-col gap-3 rounded-2xl p-4 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search practical questions..."
          ...
          <p className="text-sm text-muted">{total} coding questions in this view</p>
              ...
              : questions.length === 0
                ? (
                    <div className="glass-panel rounded-2xl p-10 text-center">
                      <p className="font-display text-xl font-semibold">No coding questions here</p>
                      <p className="mt-2 text-sm text-muted">
                        Try another topic or switch the filter to All.
                      </p>
                    </div>
                  )
                : questions.map((q, idx) => (
                    ...
                    >
                      <button
                        ...
                        title={q.codeCompleted ? 'Mark as not completed' : 'Mark as completed'}
                        aria-label={q.codeCompleted ? 'Uncheck completed' : 'Mark completed'}
                      >
                        {q.codeCompleted ? (
